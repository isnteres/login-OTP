  ...
  const NavItem = ({ id, label, icon, needsAuth = false }) => {
    ...
    return (
      <button
        ...
        onMouseLeave={(e) => {
          if (!active_) e.currentTarget.style.background = "transparent";
        }}
      >
        <span style={{ opacity: 0.65, flexShrink: 0, display: "flex" }}>
          {icon}
        </span>
        {label}
      </button>
    ...
    >
      ...
      <nav style={{ padding: "4px 8px", flex: 1 }}>
        ...
        <SectionHeader
          ...
          isOpen={openOps}
          toggle={() => setOpenOps((p) => !p)}
        />
        {openOps && (
          <>
            <NavItem
              id="operaciones_cursos"
              label="Cursos Online"
              icon={<FiBookOpen size={14} />}
            />
            <NavItem
              id="operaciones_analitica"
              label="Analítica Cursos"
        ...
        <SectionHeader
          ...
          isOpen={openSist}
          toggle={() => setOpenSist((p) => !p)}
        />
        {openSist && (
          <>
            <NavItem
              id="sistema_analitica"
              label="Analítica Web"
              icon={<FiActivity size={14} />}
              needsAuth
            />
            <NavItem
              id="sistema_auditoria"
              label="Auditoría"
...
function renderPage(active, setActive, superAdmin) {
  if (active === "sistema_analitica_gate") {
    return (
      <SuperAdminGate
        ...
        onSuccess={() => {
          superAdmin.grant("sistema_analitica");
          setActive("sistema_analitica");
        }}
      />
    );
  }
  if (active === "sistema_auditoria_gate") {
    return (
      <SuperAdminGate
        verify={superAdmin.verify}
        onSuccess={() => {
          superAdmin.grant("sistema_auditoria");
          setActive("sistema_auditoria");
        }}
      />
    );
  }
  ...
  switch (active) {
    ...
    // ── COMERCIAL (CRM) ────────────────────────────────────────────────────
    case "crm":
    case "crm_rrhh_personal":
      return <PersonalPage setActive={setActive} />;
    case "crm_rrhh_desempeno":
      return <DesempenoPage setActive={setActive} />;
    case "crm_rrhh_objetivos":
      return <ObjetivosPage setActive={setActive} />;
    case "crm_rrhh_auditoria":
      return <AuditoriaRrhhPage setActive={setActive} />;
    case "crm_soporte":
      return <SoportePage setActive={setActive} />;
    case "crm_comunidad":
    ...
    // ── SISTEMA Y SEGURIDAD ────────────────────────────────────────────────
    case "sistema_analitica":
      return <AnaliticaPage />;
    case "sistema_auditoria":
      return <AuditoriaSistemaPage />;
  ...
  return (
    <div
      style={{
        display: "flex",
        minHeight: "100vh",
        background: "linear-gradient(135deg,#0c1220,#111827,#0c1220)",
      }}
    >
      <div
        style={{
